...
def main():
    ...
    opts.exp = opts.dataset.lower() + "_" + opts.model + "_" + opts.exp + '_sat_advt'
    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            if opts.loss_settings == 1:
                ...
                # setting2 0.5 adv loss + 0.5 others
                loss = 0.5 * loss0 + 0.25 * (loss1 + loss2)
            elif opts.loss_settings == 3:
                # setting3 0.8 adv loss + 0.2 others
                loss = 0.8 * loss0 + 0.1 * (loss1 + loss2)
            elif opts.loss_settings == 4:
                # setting3 0.9 adv loss + 0.1 others
                loss = 0.9 * loss0 + 0.05 * (loss1 + loss2)
            else : assert False

            # if opts.loss_settings == 1:
            #     loss = 0.5 * ((loss0 + loss1 + loss2) / 3.0) + 0.5 * loss3
            # elif opts.loss_settings == 2:
            #     loss = (0.5 * (loss0 + loss3) + loss1 + loss2) / 3.0
            # elif opts.loss_settings == 3:
            #     loss = 0.3 * (loss0 + loss3) + 0.2 * (loss1 + loss2)
            # elif opts.loss_settings == 4:
            #     loss = 0.4 * (loss0 + loss3) + 0.1 * (loss1 + loss2)
            # else : assert False
