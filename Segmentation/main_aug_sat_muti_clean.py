    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            feature_adv2 = attack_algo.PGD(x=feature_map2, image_batch=images, low_level_feat=low_level_feat, criterion=criterion, y=labels, model=model,
                steps=1, eps=(2.0 / 255),  gamma=(0.01 / 255), idx=2)
            ...
            feature_adv3 = attack_algo.PGD(x=feature_map3, image_batch=images, low_level_feat=low_level_feat, criterion=criterion, y=labels, model=model,
                steps=1, eps=(2.0 / 255),  gamma=(0.001 / 255), idx=3)
            ...
            adv_list = attack_algo.get_sample_points(feature_map2, feature_adv2, 3)
            adv_list[2] = attack_algo.mix_feature(feature_map2, adv_list[2])
            ...
            adv_input_se_dict21 = {'x': images, 'adv': adv_list[1],         'out_idx': 2, 'flag':'tail', 'low_level_feat': low_level_feat}
            adv_input_se_dict22 = {'x': images, 'adv': adv_list[2],         'out_idx': 2, 'flag':'tail', 'low_level_feat': low_level_feat}
            ...
            output2 = model(adv_input_se_dict21)
            output3 = model(adv_input_se_dict22)
            output4 = model(adv_input_se_dict3)
            output5 = model(adv_input_se_dict4)
            output6 = model(adv_input_sd_dict)
            
            # 1108
            loss0 = criterion(output0, labels) # clean
            loss1 = criterion(output1, labels) # muti1
            loss2 = criterion(output2, labels) # muti2 + sat3 1
            loss3 = criterion(output3, labels) # muti2 + sat3 2
            loss4 = criterion(output4, labels) # muti3
            loss5 = criterion(output5, labels) # muti4
            loss6 = criterion(output6, labels) # sd


            if opts.loss_settings == 1:
                # setting1: 0.8 clean + 0.2 others
                loss = 0.8 * (loss0 + loss2 + loss3) / 3.0 + 0.2 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 2:
                # setting2: 0.6 advt + 0.4 others
                loss = 0.9 * (loss0 + loss2 + loss3) / 3.0 + 0.1 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 3:
                # setting3: 0.6 advt + 0.3 best + 0.1 others
                loss = 0.7 * (loss0 + loss2 + loss3) / 3.0 + 0.3 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 4:
                # setting4: 0.6 advt + 0.2 best + 0.2 others
                loss = 0.6 * (loss0 + loss2 + loss3) / 3.0 + 0.4 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 5:
                # setting5: 0.6 advt + 0.4 others
                loss = 0.8 * loss0 + 0.1 * (loss2 + loss3) / 2.0 + 0.1 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 6:
                # setting5: 0.6 advt + 0.4 others
                loss = 0.6 * loss0 + 0.3 * (loss2 + loss3) / 2.0 + 0.1 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 7:
                # setting5: 0.6 advt + 0.4 others
                loss = 0.5 * loss0 + 0.4 * (loss2 + loss3) / 2.0 + 0.1 * (loss1 + loss4 + loss5 + loss6) / 4.0
            elif opts.loss_settings == 8:
                # setting5: 0.6 advt + 0.4 others
                loss = 0.4 * loss0 + 0.5 * (loss2 + loss3) / 2.0 + 0.1 * (loss1 + loss4 + loss5 + loss6) / 4.0
            else: assert False

            # if opts.loss_settings == 1:
            #     # setting1: 0.8 clean + 0.2 others
            #     loss = 0.8 * loss0 + 0.2 * (loss1 + loss2 + loss3 + loss4 + loss5 + loss6) / 6.0
            # elif opts.loss_settings == 2:
            #     # setting2: 0.6 advt + 0.4 others
            #     loss = 0.6 * loss0 + 0.4 * (loss1 + loss2 + loss3 + loss4 + loss5 + loss6)  / 6.0
            # elif opts.loss_settings == 3:
            #     # setting3: 0.6 advt + 0.3 best + 0.1 others
            #     loss = 0.6 * loss0 + 0.3 * loss3 + 0.1 * (loss1 + loss2 + loss4 + loss5 + loss6) / 5.0
            # elif opts.loss_settings == 4:
            #     # setting4: 0.6 advt + 0.2 best + 0.2 others
            #     loss = 0.6 * loss0 + 0.2 * loss3 + 0.2 * (loss1 + loss2 + loss4 + loss5 + loss6) / 5.0
            # else: assert False
