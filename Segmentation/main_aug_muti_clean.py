...
def main():
    ...
    opts.exp = opts.dataset.lower() + "_" + opts.model + "_" + opts.exp + "_muti_clean" 
    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            # muti loss 
            loss1 = criterion(output1, labels) # layer1
            loss2 = criterion(output2, labels) # layer2
            loss3 = criterion(output3, labels) # layer3 best
            loss4 = criterion(output4, labels) # layer4
            loss5 = criterion(output5, labels) # sd

            if opts.loss_settings == 1:
                # setting1: 0.8 (clean + best) + 0.2 others
                loss = 0.4 * (loss0 + loss3) + 0.05 * (loss1 + loss2 + loss4 + loss5)
            elif opts.loss_settings == 2:
                # setting2: 0.9 (clean + best + sd)+ 0.1 others
                loss = 0.3 * (loss0 + loss3 + loss5) + 0.1 * (loss1 + loss2 + loss4) / 3.0
            elif opts.loss_settings == 3:
                # setting3: 0.4 (clean) + 0.4 (best + sd) + 0.2 others
                loss = 0.4 * loss0 + 0.2 * (loss3 + loss5) + 0.2 * (loss1 + loss2 + loss4) / 3.0
            elif opts.loss_settings == 4:
                # setting4: 0.3 (clean + layer3 + layer4) + 0.1 others
                loss = 0.4 * loss0 + 0.3 * loss3 + 0.2 * loss5 + 0.1 * (loss1 + loss2 + loss4) / 3.0
            else: assert False

            # if opts.loss_settings == 1:
            #     # setting1: average all loss
            #     loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5) / 6.0
            # elif opts.loss_settings == 2:
