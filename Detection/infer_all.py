...
def _infer(path_to_input_image: str, path_to_output_image: str, path_to_checkpoint: str, dataset_name: str, backbone_name: str, prob_thresh: float):
    ...
    all_images = os.listdir(path_to_input_image)

    for i, img in enumerate(all_images):
        image_path = os.path.join(path_to_input_image, img)
        with torch.no_grad():
            ...
            inputs_all = {"x": image_tensor, "adv": None, "out_idx": None, "flag": 'clean'}
            ...
            save_path = os.path.join(path_to_output_image, img)
            image.save(save_path)
            print("finish:[{}/{}]".format(i, len(all_images)))
