...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            # clean best loss: 75.11
            loss_clean_adv = 0.2 * (loss0 + loss1 + loss2 + loss3 + loss4)
            
            if args.loss_settings == 1:
                # setting 1 : total our clean loss
                loss = loss_clean_adv
            elif args.loss_settings == 2:
                # setting 2 : 0.4 clean + 0.5 adv loss
                loss = 0.5 * loss_clean_adv + 0.5 * loss0
            elif args.loss_settings == 3:
                # setting 3 : 0.4 clean + 0.6 adv loss
                loss = 0.4 * loss_clean_adv + 0.6 * loss0
            elif args.loss_settings == 4:
                # setting 4 : 0.3 clean + 0.7 adv loss
                loss = 0.3 * loss_clean_adv + 0.7 * loss0
            else: assert False

            # # adv + clean
            # if args.loss_settings == 1:
            #     # setting 1 : 0.5 sat clean + 0.5 advt
            #     loss = 0.5 * (0.2 * (loss0 + loss1 + loss2 + loss3 + loss4)) + 0.5 * loss5
            # elif args.loss_settings == 2:
            #     # setting 2
            #     loss = 0.2 * (0.5 * (loss0 + loss5) + loss1 + loss2 + loss3 + loss4)
            # elif args.loss_settings == 3:
            #     # setting 3
            #     loss = 0.4 * (loss0 + loss5) + 0.05 * (loss1 + loss2 + loss3 + loss4)
            # elif args.loss_settings == 4:
            #     # setting 4
            #     loss = 0.3 * (loss0 + loss5) + 0.1 * (loss1 + loss2 + loss3 + loss4)
            # else: assert False
