...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            feature_adv3 = attack_algo.PGD(feature_map3, image_batch, y=y, model=model, steps=1, eps=(2.0 / 255), gamma=(0.2   / 255), idx=3)
            ...
            loss_clean_adv = 0.35 * (loss0 + loss3) + 0.1 * (loss1 + loss2 + loss4)
            
            if args.loss_settings == 1:
                # setting 1 : total our loss
                loss = loss_clean_adv
            elif args.loss_settings == 2:
                # setting 2 : 0.5 * total our loss + 0.5 adv loss
                loss = 0.5 * loss_clean_adv + 0.5 * loss0
            elif args.loss_settings == 3:
                # setting 3 : 0.4 * total our loss + 0.6 adv loss
                loss = 0.4 * loss_clean_adv + 0.6 * loss0
            elif args.loss_settings == 4:
                # setting 4 : 0.3 * total our loss + 0.7 adv loss
                loss = 0.3 * loss_clean_adv + 0.7 * loss0
            else: assert False

            # if args.loss_settings == 1:
            #     # setting 1 : 0.5 muti clean + 0.5 adv [62.51]
            #     loss = 0.5 * (0.35 * (loss0 + loss3) + 0.1 * (loss1 + loss2 + loss4)) + 0.5 * loss5
            # elif args.loss_settings == 2:
            #     # setting 2 : 
            #     loss = 0.3 * (loss0 + loss3 + loss5) + 0.03333 * (loss1 + loss2 + loss4)
            # elif args.loss_settings == 3:
            #     # setting 3 : (adv + clean) * 0.5
            #     loss = 0.35 * (0.5 * (loss0 + loss5) + loss3) + 0.1 * (loss1 + loss2 + loss4)
            # elif args.loss_settings == 4:
            #     # setting 4
            #     loss = 0.25 * (loss0 + loss5) + 0.125 * (loss1 + loss2 + loss3 + loss4)
            # else: assert False
...
if __name__ == '__main__':
    def main():
        ...
        print("INFO: Loss Setting:[{}]".format(args.loss_settings))
        print("-" * 100)
