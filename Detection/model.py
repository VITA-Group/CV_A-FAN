...
class Model(nn.Module):
    ...
    def forward(self, input_dict: dict,
        ...
        if input_dict['flag'] == 'head':
            
            for bn_module in self._bn_modules:
                bn_module.eval()

            feature_map = self.features(input_dict)
            return feature_map

        elif input_dict['flag'] == 'tail' or input_dict['flag'] == 'clean':

            
            if self.training:
                # Type : not rpn and roi 
                if type(input_dict['out_idx']) == int:

                    for bn_module in self._bn_modules:
                        bn_module.eval()
                    
                    features = self.features(input_dict)
                    batch_size, _, image_height, image_width = input_dict['x'].shape
                    _, _, features_height, features_width = features.shape
                    anchor_bboxes = self.rpn.generate_anchors(image_width, image_height, 
                        num_x_anchors=features_width, num_y_anchors=features_height).to(features).repeat(batch_size, 1, 1)
                    
                    anchor_objectnesses, anchor_transformers, anchor_objectness_losses, anchor_transformer_losses = self.rpn.forward(features, 
                        anchor_bboxes, gt_bboxes_batch, image_width, image_height)
                    proposal_bboxes = self.rpn.generate_proposals(anchor_bboxes, anchor_objectnesses, anchor_transformers, image_width, image_height).detach()
                    proposal_classes, proposal_transformers, proposal_class_losses, proposal_transformer_losses = self.detection.forward(features, 
                        proposal_bboxes, gt_classes_batch, gt_bboxes_batch)

                    return anchor_objectness_losses, anchor_transformer_losses, proposal_class_losses, proposal_transformer_losses

                elif input_dict['out_idx'] == 'rpn_head':
                    ...
                    anchor_bboxes = self.rpn.generate_anchors(image_width, image_height, 
                        num_x_anchors=features_width, num_y_anchors=features_height).to(features).repeat(batch_size, 1, 1)
                    rpn_feature_map_dict = self.rpn.forward(features, anchor_bboxes, gt_bboxes_batch, image_width, image_height, return_type='head')
                    
                    return_dict['features'] = features
                    return_dict['image_height'] = torch.tensor([[image_height]]).cuda()
                    return_dict['image_width'] = torch.tensor([[image_width]]).cuda()
                    return_dict['anchor_bboxes'] = anchor_bboxes
                    return_dict['rpn_feature_map_dict'] = rpn_feature_map_dict
                    
                    return return_dict

                elif input_dict['out_idx'] == 'rpn_tail':

                    use_full_dict = input_dict['adv']
                    features = use_full_dict['features']
                    image_width = use_full_dict['image_width'][0].item()
                    image_height = use_full_dict['image_height'][0].item()
                    anchor_bboxes = use_full_dict['anchor_bboxes']
                    rpn_feature_map_dict = use_full_dict['rpn_feature_map_dict']

                    anchor_objectnesses, anchor_transformers, anchor_objectness_losses, anchor_transformer_losses = self.rpn.forward(rpn_feature_map_dict, 
                        anchor_bboxes, gt_bboxes_batch, image_width, image_height, return_type='tail')
                    proposal_bboxes = self.rpn.generate_proposals(anchor_bboxes, anchor_objectnesses, anchor_transformers, image_width, image_height).detach()  
                    proposal_classes, proposal_transformers, proposal_class_losses, proposal_transformer_losses = self.detection.forward(features, 
                        proposal_bboxes, gt_classes_batch, gt_bboxes_batch)
                ...
                elif input_dict['out_idx'] == 'roi_head':
                    ...
                    roi_output_dict = self.detection.forward(features, proposal_bboxes, gt_classes_batch, gt_bboxes_batch, return_type='head')

                    return_dict['anchor_objectness_losses'] = anchor_objectness_losses
                    return_dict['anchor_transformer_losses'] = anchor_transformer_losses
                    return_dict['roi_output_dict'] = roi_output_dict

                    return return_dict


                elif input_dict['out_idx'] == 'roi_tail':
                    
                    use_full_dict = input_dict['adv']
                    anchor_objectness_losses = use_full_dict['anchor_objectness_losses']
                    anchor_transformer_losses = use_full_dict['anchor_transformer_losses']
                    roi_output_dict = use_full_dict['roi_output_dict']

                    proposal_classes, proposal_transformers, proposal_class_losses, proposal_transformer_losses = self.detection.forward(roi_output_dict, 
                        return_type='tail')
                ...
                if input_dict['flag'] == 'head':
                ...
                elif input_dict['flag'] == 'clean' or input_dict['flag'] == 'tail':
                    ...
                    detection_bboxes, detection_classes, detection_probs, detection_batch_indices = self.detection.generate_detections(proposal_bboxes, 
                        proposal_classes, proposal_transformers, image_width, image_height)
    ...
    def load(self, path_to_checkpoint: str, optimizer: Optimizer = None, scheduler: _LRScheduler = None, convert=False) -> 'Model':
        
        checkpoint = torch.load(path_to_checkpoint)
        if convert:
            checkpoint['state_dict'] = convert_dict(checkpoint['state_dict'])
        # self.load_state_dict(checkpoint['state_dict'])
        model_state_dict = self.state_dict()
        overlap_dict = {k : v for k, v in checkpoint['state_dict'].items() if k in model_state_dict.keys()}
        model_state_dict.update(overlap_dict)
        self.load_state_dict(model_state_dict)
        print("Load Weight:[{}/{}], DIR:[{}]"
        .format(len(overlap_dict.keys()), len(model_state_dict.keys()), path_to_checkpoint))
    ...
    class Detection(nn.Module):
        ...
        def forward(self, features, proposal_bboxes: Tensor = None,
                    gt_classes_batch: Optional[Tensor] = None, gt_bboxes_batch: Optional[Tensor] = None, return_type = 'clean') -> Union[Tuple[Tensor, Tensor], Tuple[Tensor, Tensor, Tensor, Tensor]]:
            ...
            if return_type == 'clean':
                ...
                return_dict['roi_feature_map'] = hidden
                return_dict['gt_proposal_classes'] = gt_proposal_classes
                return_dict['gt_proposal_transformers'] = gt_proposal_transformers
                return_dict['batch_size'] = torch.tensor([[batch_size]]).cuda()
                return_dict['batch_indices'] = batch_indices

                return return_dict

            elif return_type == 'tail':
                
                hidden = features['roi_feature_map']
                ...
                proposal_transformers = self._proposal_transformer(hidden)
                proposal_class_losses, proposal_transformer_losses = self.loss(proposal_classes, proposal_transformers,
                                                                            features['gt_proposal_classes'], 
                                                                            features['gt_proposal_transformers'],
                                                                            batch_size, 
...
def convert_dict(ori_dict):
    ...
    for k, v in ori_dict.items():
        if "features.0." in k and "rpn" not in k:
            j = k.replace("0", "conv1")
        elif "features.1." in k:
            j = k.replace("1", "bn1")
        elif "features.4" in k:
            j = k.replace("4", "layer1")
        elif "features.5" in k:
            j = k.replace("5", "layer2")
        elif "features.6" in k:
            j = k.replace("6", "layer3") 
        else:
            j = k
        new_dict[j] = v
    return new_dict