...
def _eval(path_to_checkpoint: str, dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_results_dir: str, args):
    ...
    model = Model(backbone, dataset.num_classes(), pooler_mode=Config.POOLER_MODE,
                  ...
                  anchor_smooth_l1_loss_beta=1.0, proposal_smooth_l1_loss_beta=1.0).cuda()
    
    model.load(path_to_checkpoint, convert=args.convert)

    Log.i('Start evaluating Robustness with 1 GPU (1 batch per GPU)')

    origin_weight = model.state_dict()
    model_state_dict = model.state_dict()


    final_loss_list = []
    for perturb_iter in range(10):
        perturb_dict = attack_algo.perturb_weight(model.state_dict())
        print("=================================================================")
        loss_list = []
        for i in range(20):
            t = i / 10.0
            new_dict = {}
            for key in origin_weight.keys():
                if key in perturb_dict.keys():
                    new_dict[key] = origin_weight[key] + t * perturb_dict[key]
                else:
                    new_dict[key] = origin_weight[key]
            
            overlap_dict = {k : v for k, v in new_dict.items() if k in origin_weight.keys()}
            model_state_dict.update(overlap_dict)
            model.load_state_dict(model_state_dict)
            final_loss = evaluator.evaluate_loss(model, args)
            print("t={}, loss={}".format(t, final_loss))
...
if __name__ == '__main__':
    def main():
        ...
        #PGD Setting 
        parser.add_argument('--steps', default=10, type=int, help='PGD-steps')
        ...
        parser.add_argument('--convert', action="store_true", help="whether using apex")
        ...
        _eval(path_to_checkpoint, dataset_name, backbone_name, path_to_data_dir, path_to_results_dir, args)
