    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if args.loss_settings == 1:
                ...
                # setting 2
                loss = 0.6 * loss0 + 0.1 * (loss1 + loss2 + loss3 + loss4)
            elif args.loss_settings == 3:
                # setting 3
                loss = 0.5 * loss0 + 0.2 * loss1 + 0.2 * loss2 + 0.05 * loss3 + 0.05 * loss4
            elif args.loss_settings == 4:
                # setting 4
                loss = 0.5 * loss0 + 0.05 * loss1 + 0.05 * loss2 + 0.2 * loss3 + 0.2 * loss4
            else: assert False
