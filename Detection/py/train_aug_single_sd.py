...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            inputs_all = {"x": image_batch, "adv": None, "out_idx": args.pertub_idx + '_head', "flag": 'clean'}
            rpn_roi_output_dict = model.train().forward(inputs_all, bboxes_batch, labels_batch)
            clean_feature_map = rpn_roi_output_dict['roi_output_dict']['roi_feature_map'].detach()

            adv_rpn_roi_output_dict = attack_algo.rpn_roi_PGD(
                layer=args.pertub_idx,
                ...
                clip = args.clip,
                only_roi_loss = args.only_roi)

            adv_feature_map = adv_rpn_roi_output_dict['roi_output_dict']['roi_feature_map'].detach()
            
            if args.mix:
                adv_feature_map = attack_algo.mix_feature(clean_feature_map, adv_feature_map)
            if args.noise != 0:
                adv_feature_map += (2.0 * torch.rand(adv_feature_map.shape).cuda() - 1.0) * args.gamma * args.noise

            adv_rpn_roi_output_dict['roi_output_dict']['roi_feature_map'] = adv_feature_map
            adv_input_dict = {'adv': adv_rpn_roi_output_dict, 'out_idx': args.pertub_idx + '_tail', 'flag':'clean'}
            ...
            # adv + clean
            loss = args.adv_loss_weight * loss1 + (1 - args.adv_loss_weight) * loss2
            
            anchor_objectness_loss = anchor_objectness_loss1 + anchor_objectness_loss2
            anchor_transformer_loss = anchor_transformer_loss1 + anchor_transformer_loss2
            proposal_class_loss = proposal_class_loss1 + proposal_class_loss2
            proposal_transformer_loss = proposal_transformer_loss1 + proposal_transformer_loss2
...
if __name__ == '__main__':
    def main():
        ...
        parser.add_argument('--pertub_idx', help='index of perturb layers', default=None, type=str)
        ...
        parser.add_argument('--noise', help='if use noise', default=0, type=float)
        parser.add_argument('--adv_loss_weight', help='loss', default=0.5, type=float)
        parser.add_argument('--only_roi', action="store_true", help="whether only using roi loss")
        parser.add_argument('--mix', action="store_true", help="whether using mix")
        ...
        exp_name = "s" + str(args.steps) + "p_" + str(args.pertub_idx) + "_g" + str(args.gamma) + "e" + str(args.eps)
        ...
        print("EXP: Dataset:[{}] ADV Step:[{}] Layer:[{}] Gamma:[{}] Eps:[{}] Randinit:[{}] Clip:[{}] Only ROI:[{}] MIX:[{}] Noise:[{}] Adv Loss Weight:[{}]"
            .format(args.dataset, args.steps, args.pertub_idx, args.gamma, args.eps, args.randinit, 
                    args.clip, args.only_roi, args.mix, args.noise, args.adv_loss_weight))
        print("-" * 100)
