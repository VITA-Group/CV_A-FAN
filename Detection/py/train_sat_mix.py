...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            mix_feature1 = attack_algo.mix_feature(feature_map, adv_list[1])
            mix_feature2 = attack_algo.mix_feature(feature_map, adv_list[2])
            mix_feature3 = attack_algo.mix_feature(feature_map, adv_list[3]) 
            mix_feature4 = attack_algo.mix_feature(feature_map, adv_list[4])
            
            clean_input_dict = {'x': image_batch, "adv": None, 'out_idx': 0, 'flag':'clean'}
            adv_input_dict1 = {'x': image_batch, 'adv': mix_feature1, 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict2 = {'x': image_batch, 'adv': mix_feature2, 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict3 = {'x': image_batch, 'adv': mix_feature3, 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict4 = {'x': image_batch, 'adv': mix_feature4, 'out_idx': args.pertub_idx, 'flag':'tail'}
