...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            clean_roi_feature_map = rpn_roi_output_dict['roi_output_dict']['roi_feature_map'].detach()
            ...
            adv_roi_feature_map = adv_rpn_roi_output_dict['roi_output_dict']['roi_feature_map'].detach()
            adv_list_sd = attack_algo.get_sample_points(clean_roi_feature_map, adv_roi_feature_map, 5)

            clean_input_dict = {'x': image_batch, "adv": None, 'out_idx': 0, 'flag':'clean'}
            adv_input_dict_sd = {'adv': adv_rpn_roi_output_dict, 'out_idx': args.pertub_idx + '_tail', 'flag':'clean'}
            ...
            adv_input_dict_sd['adv']['roi_output_dict']['roi_feature_map'] = adv_list_sd[1]
            ...
            loss = (loss0 + loss5 + loss6 + loss7 + loss8) / 5.0
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            scheduler.step()
...
if __name__ == '__main__':
    def main():
        ...
        print("EXP: [SD + SAT] Dataset:[{}] Step:[{}] Layer:[{}] Gamma:[{}] Eps:[{}] Randinit:[{}] Clip:[{}]"
