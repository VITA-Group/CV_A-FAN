...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if f1:
                adv_list[1] = attack_algo.mix_feature(feature_map, adv_list[1])
            if f2:
                adv_list[2] = attack_algo.mix_feature(feature_map, adv_list[2])
            if f3:
                adv_list[3] = attack_algo.mix_feature(feature_map, adv_list[3])
            if f4:
                adv_list[4] = attack_algo.mix_feature(feature_map, adv_list[4])
            
            clean_input_dict = {'x': image_batch, "adv": None, 'out_idx': 0, 'flag':'clean'}
            adv_input_dict1 = {'x': image_batch, 'adv': adv_list[1], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict2 = {'x': image_batch, 'adv': adv_list[2], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict3 = {'x': image_batch, 'adv': adv_list[3], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict4 = {'x': image_batch, 'adv': adv_list[4], 'out_idx': args.pertub_idx, 'flag':'tail'}
            ...
            anchor_objectness_losses4, anchor_transformer_losses4, proposal_class_losses4, proposal_transformer_losses4 = \
                model.train().forward(adv_input_dict4, bboxes_batch, labels_batch)

            # clean loss
            anchor_objectness_loss0 = anchor_objectness_losses0.mean()
            anchor_transformer_loss0 = anchor_transformer_losses0.mean()
            proposal_class_loss0 = proposal_class_losses0.mean()
            proposal_transformer_loss0 = proposal_transformer_losses0.mean()
            loss0 = anchor_objectness_loss0 + anchor_transformer_loss0 + proposal_class_loss0 + proposal_transformer_loss0
            ...
            # adv3 loss
            anchor_objectness_loss4 = anchor_objectness_losses4.mean()
            anchor_transformer_loss4 = anchor_transformer_losses4.mean()
            proposal_class_loss4 = proposal_class_losses4.mean()
            proposal_transformer_loss4 = proposal_transformer_losses4.mean()
            loss4 = anchor_objectness_loss4 + anchor_transformer_loss4 + proposal_class_loss4 + proposal_transformer_loss4

            # adv + clean
            loss = 0.2 * (loss0 + loss1 + loss2 + loss3 + loss4)
            
            anchor_objectness_loss = (anchor_objectness_loss0 + anchor_objectness_loss1 + anchor_objectness_loss2 + anchor_objectness_loss3  + anchor_objectness_loss4) * 0.2
            anchor_transformer_loss = (anchor_transformer_loss0 + anchor_transformer_loss1 + anchor_transformer_loss2 + anchor_transformer_loss3 + anchor_transformer_loss4) * 0.2
            proposal_class_loss = (proposal_class_loss0 + proposal_class_loss1 + proposal_class_loss2 + proposal_class_loss3 + proposal_class_loss4) * 0.2
            proposal_transformer_loss = (proposal_transformer_loss0 + proposal_transformer_loss1 + proposal_transformer_loss2 + proposal_transformer_loss3 + proposal_transformer_loss4) * 0.2
...
if __name__ == '__main__':
    def main():
        ...
        exp_name = "s" + str(args.steps) + "p" + str(args.pertub_idx) + "g" + str(args.gamma) + "e" + str(args.eps) + "_" + args.mix_layer
        ...
        print("-" * 100)
        print("INFO: PID   : [{}]".format(os.getpid()))
        print("EXP: Dataset:[{}] ADV Step:[{}] Layer:[{}] Gamma:[{}] Eps:[{}] Randinit:[{}] Clip:[{}] MIX:[{}]"
            .format(args.dataset, args.steps, args.pertub_idx, args.gamma, args.eps, args.randinit, args.clip, args.mix_layer))
