...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            inputs_all_se = {"x": image_batch, "adv": None, "out_idx": 3,           "flag": 'head'}
            inputs_all_m1 = {"x": image_batch, "adv": None, "out_idx": 1,           "flag": 'head'}
            inputs_all_m2 = {"x": image_batch, "adv": None, "out_idx": 2,           "flag": 'head'}
            # get all feature maps
            feature_map_se = model.train().forward(inputs_all_se, bboxes_batch, labels_batch).detach()
            feature_map_m1 = model.train().forward(inputs_all_m1, bboxes_batch, labels_batch).detach()
            feature_map_m2 = model.train().forward(inputs_all_m2, bboxes_batch, labels_batch).detach()
            ...
            # se best mix feature
            feature_adv_se = attack_algo.PGD(feature_map_se, image_batch, y=y, model=model, steps=1, eps=(2.0 / 255), gamma=(0.3 / 255), idx=3)
            feature_adv_se = attack_algo.mix_feature(feature_map_se, feature_adv_se)

            # sd best mix feature
            adv_rpn_roi_output_dict = attack_algo.rpn_roi_PGD(rpn_roi_output_dict = rpn_roi_output_dict, y=y, model=model, 
                                                              steps=1, eps=(2.0 / 255), gamma=(0.2 / 255), only_roi_loss=False)
            ...
            # muti mix feature
            feature_adv_m1 = attack_algo.PGD(feature_map_se, image_batch, y=y, model=model, steps=1, eps=(2.0 / 255), gamma=(0.001 / 255), idx=1)
            feature_adv_m2 = attack_algo.PGD(feature_map_se, image_batch, y=y, model=model, steps=1, eps=(2.0 / 255), gamma=(0.001 / 255), idx=2)
