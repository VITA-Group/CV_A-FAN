...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str], args):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            adv_list = attack_algo.get_sample_points(feature_map, feature_adv, 10)
            ...
            adv_input_dict5 = {'x': image_batch, 'adv': adv_list[5], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict6 = {'x': image_batch, 'adv': adv_list[6], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict7 = {'x': image_batch, 'adv': adv_list[7], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict8 = {'x': image_batch, 'adv': adv_list[8], 'out_idx': args.pertub_idx, 'flag':'tail'}
            adv_input_dict9 = {'x': image_batch, 'adv': adv_list[9], 'out_idx': args.pertub_idx, 'flag':'tail'}
            ...
            anchor_objectness_losses5, anchor_transformer_losses5, proposal_class_losses5, proposal_transformer_losses5 = \
                model.train().forward(adv_input_dict5, bboxes_batch, labels_batch)
            anchor_objectness_losses6, anchor_transformer_losses6, proposal_class_losses6, proposal_transformer_losses6 = \
                model.train().forward(adv_input_dict6, bboxes_batch, labels_batch)
            anchor_objectness_losses7, anchor_transformer_losses7, proposal_class_losses7, proposal_transformer_losses7 = \
                model.train().forward(adv_input_dict7, bboxes_batch, labels_batch)
            anchor_objectness_losses8, anchor_transformer_losses8, proposal_class_losses8, proposal_transformer_losses8 = \
                model.train().forward(adv_input_dict8, bboxes_batch, labels_batch)
            anchor_objectness_losses9, anchor_transformer_losses9, proposal_class_losses9, proposal_transformer_losses9 = \
                model.train().forward(adv_input_dict9, bboxes_batch, labels_batch)
            ...
            loss8 = attack_algo.compute_loss(anchor_objectness_losses8, anchor_transformer_losses8, proposal_class_losses8, proposal_transformer_losses8)
            loss9 = attack_algo.compute_loss(anchor_objectness_losses9, anchor_transformer_losses9, proposal_class_losses9, proposal_transformer_losses9)
            
            # adv + clean
            loss = 0.1 * (loss0 + loss1 + loss2 + loss3 + loss4 + loss5 + loss6 + loss7 + loss8 + loss9)
