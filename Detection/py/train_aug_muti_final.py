    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if args.loss_settings == 1:
                # setting 1: 75.11
                loss = 0.35 * (loss0 + loss3) + 0.1 * (loss1 + loss2 + loss4)
            elif args.loss_settings == 2:
                # setting 2: 74.58
                loss = 0.2 * (loss0 + loss3 + loss1 + loss2 + loss4)
            elif args.loss_settings == 3:
                # setting 3
                loss = 0.3 * (loss0 + loss3 + loss4) + 0.05 * (loss1 + loss2)
            elif args.loss_settings == 4:
                # setting 4
                loss = 0.5 * loss0 + 0.125 * (loss1 + loss2 + loss3 + loss4)
            else: assert False
