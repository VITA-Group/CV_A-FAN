...
class VOC20072012(Base):
    ...
    def __init__(self, path_to_data_dir: str, mode: Base.Mode, image_min_side: float, image_max_side: float):
        ...
        path_to_voc2012_dir = os.path.join(self._path_to_data_dir, 'VOCdevkit', 'VOC2012')

        path_to_imagesets_main_dir_2007 = os.path.join(path_to_voc2007_dir, 'ImageSets', 'Main')
        path_to_imagesets_main_dir_2012 = os.path.join(path_to_voc2012_dir, 'ImageSets', 'Main')

        path_to_annotations_dir_2007 = os.path.join(path_to_voc2007_dir, 'Annotations')
        path_to_annotations_dir_2012 = os.path.join(path_to_voc2012_dir, 'Annotations')

        self._path_to_jpeg_images_dir_2007 = os.path.join(path_to_voc2007_dir, 'JPEGImages')
        self._path_to_jpeg_images_dir_2012 = os.path.join(path_to_voc2012_dir, 'JPEGImages')


        if self._mode == VOC20072012.Mode.TRAIN:
            path_to_image_ids_txt_2007 = os.path.join(path_to_imagesets_main_dir_2007, 'trainval.txt')
            path_to_image_ids_txt_2012 = os.path.join(path_to_imagesets_main_dir_2012, 'trainval.txt')
        elif self._mode == VOC20072012.Mode.EVAL:
            path_to_image_ids_txt_2007 = os.path.join(path_to_imagesets_main_dir_2007, 'test.txt')
        else:
            raise ValueError('invalid mode')

        with open(path_to_image_ids_txt_2007, 'r') as f:
            lines = f.readlines()
            self._image_ids2007 = [line.rstrip() for line in lines]

        with open(path_to_image_ids_txt_2012, 'r') as f:
            lines = f.readlines()
            self._image_ids2012 = [line.rstrip() for line in lines]  
        
        self._image_id_to_annotation_dict = {}
        self._image_ratios = []

        for image_id in self._image_ids2007:
            path_to_annotation_xml2007 = os.path.join(path_to_annotations_dir_2007, f'{image_id}.xml')
            tree = ET.ElementTree(file=path_to_annotation_xml2007)
            root = tree.getroot()

            self._image_id_to_annotation_dict[image_id] = VOC20072012.Annotation(
                filename=root.find('filename').text,
                objects=[VOC20072012.Annotation.Object(
        ...
        for image_id in self._image_ids2012:
            path_to_annotation_xml2012 = os.path.join(path_to_annotations_dir_2012, f'{image_id}.xml')
            tree = ET.ElementTree(file=path_to_annotation_xml2012)
            root = tree.getroot()
        ...
        self._image_ids = self._image_ids2007 + self._image_ids2012
    ...
    def __getitem__(self, index: int) -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        ...
        labels = [VOC20072012.CATEGORY_TO_LABEL_DICT[obj.name] for obj in annotation.objects if not obj.difficult]

        bboxes = torch.tensor(bboxes, dtype=torch.float)
        labels = torch.tensor(labels, dtype=torch.long)
        
        if annotation.filename[0] == '0':
            self._path_to_jpeg_images_dir = self._path_to_jpeg_images_dir_2007
        elif annotation.filename[0] == '2':
            self._path_to_jpeg_images_dir = self._path_to_jpeg_images_dir_2012
        else:
            assert False

        image = Image.open(os.path.join(self._path_to_jpeg_images_dir, annotation.filename))
        # random flip on only training mode
        if self._mode == VOC20072012.Mode.TRAIN and random.random() > 0.5:
            image = ImageOps.mirror(image)
            bboxes[:, [0, 2]] = image.width - bboxes[:, [2, 0]]  # index 0 and 2 represent `left` and `right` respectively

        image, scale = VOC20072012.preprocess(image, self._image_min_side, self._image_max_side)
    ...
    def evaluate(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]) -> Tuple[float, str]:
        ...
        class_to_ap_dict = {}
        for c in range(1, VOC20072012.num_classes()):
            category = VOC20072012.LABEL_TO_CATEGORY_DICT[c]
            try:
                ...
                _, _, ap = voc_eval(detpath=os.path.join(path_to_results_dir, 'comp3_det_test_{:s}.txt'.format(category)),
                                    annopath=os.path.join(path_to_annotations_dir_2007, '{:s}.xml'),
        ...
        detail = ''
        for c in range(1, VOC20072012.num_classes()):
            detail += '{:d}: {:s} AP = {:.4f}\n'.format(c, VOC20072012.LABEL_TO_CATEGORY_DICT[c], class_to_ap_dict[c])
    ...
    def _write_results(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]):
        ...
        for c in range(1, VOC20072012.num_classes()):
            class_to_txt_files_dict[c] = open(os.path.join(path_to_results_dir, 'comp3_det_test_{:s}.txt'.format(VOC20072012.LABEL_TO_CATEGORY_DICT[c])), 'w')
